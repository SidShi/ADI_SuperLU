...
int main(int argc, char *argv[])
{
    ...
    int    iam_A, iam_B, info, ldu, ldv, ldz, ldy;
    char   **cpp, c, *suffix;
    char   postfix[10];
    FILE   *fp_A, *fp_B, *fp_shift, *fp_U, *fp_V, *fp_X, *fp_F, *fopen();
    int ii, omp_mpi_level;
    int ldumap, myrank, p; /* The following variables are used for batch solves */
    int*    usermap;
    double *pp, *qq;
    double *U, *V, *Z, *D, *Y, *trueX_global, *U_global, *V_global, *trueF_global;
    double tol = 1.0/(1e10);
    int_t ll;
    float result_min[2];
    result_min[0]=1e10;
	...
	if ( **cpp == '-' ) {
        ...
        strcpy(name_U, *cpp);
        strcat(name_U, buf_U);
        strcat(name_U, postfix);
        if (!(fp_U = fopen (name_U, "r")))
        {
            ABORT ("File for RHS first factor does not exist");
        }
        ...
        {
            ABORT ("File for RHS second factor does not exist");
        }
    ...
    /* ------------------------------------------------------------
       ...
       ------------------------------------------------------------*/
    dread_shift(fp_shift, &pp, &qq, &ll, &grid_A, &grid_B);
    if (!iam_A) {
        printf("Read shifts!\n");
        fflush(stdout);
    }
    ...
    if (iam_A != -1) {
        dread_RHS_factor(fp_U, &U, &grid_A, &m_A, &r, &ldu, &U_global);
        if (!iam_A) {
            printf("Read U!\n");
            fflush(stdout);
        }
    ...
    if (iam_B != -1) {
        dread_RHS_factor(fp_V, &V, &grid_B, &m_B, &r, &ldv, &V_global);
        if (!iam_B) {
            printf("Read V!\n");
            fflush(stdout);
        }
    ...
    dtransfer_size(&m_A, &m_B, &grid_A, &grid_B);
    printf("Process %d in grid_A and %d in grid_B knows problem size %d and %d.\n", iam_A, iam_B, m_A, m_B);
    fflush(stdout);
    ...
    /* ------------------------------------------------------------
       ...
       ------------------------------------------------------------*/
    fadi(options, m_A, nnz_A, nzval_A, rowind_A, colptr_A, m_B, nnz_B, nzval_B, rowind_B, colptr_B,
        &grid_A, &grid_B, U, ldu, V, ldv, pp, qq, ll, tol, &Z, &D, &Y, r, &rank);
    ...
    /* ------------------------------------------------------------
       ...
       ------------------------------------------------------------*/
    dcheck_error_fadi(m_A, nnz_A, nzval_A, rowind_A, colptr_A, m_B, nnz_B, nzval_B, rowind_B, colptr_B, 
        &grid_A, &grid_B, trueF_global, r, Z, D, Y, rank, trueX_global);
