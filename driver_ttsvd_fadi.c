...
int main(int argc, char *argv[])
{
    ...
    int    iam_A, iam_B, iam_C, info, ldu1, ldu2, ldv2, ldt1, ldt2, ldt3;
    char   **cpp, c, *suffix;
    char   postfix[10];
    FILE   *fp_A, *fp_B, *fp_C, *fp_shift1, *fp_shift2, *fp_int, *fp_U1, *fp_U2, *fp_V2, *fp_X, *fp_F, *fp_M, *fp_R, *fopen();
    int ii, omp_mpi_level;
    int p; /* The following variables are used for batch solves */
    int_t i, j;
    double *pp1, *qq1, *pp2, *qq2;
    double *U1, *U2, *V2, *trueX_global, *U1_global, *U2_global, *V2_global, *trueF_global;
    double tol = 1.0/(1e10);
    double la, ua, lb, ub;
    int_t ll1, ll2;
    float result_min[2];
    ...
    if ((iam_B != -1) || (iam_C != -1)) {
        dread_shift_twogrids(fp_shift2, &pp2, &qq2, &ll2, &grid_B, &grid_C, 1, 2, grid_proc);
    ...
    if (iam_B != -1) {
        dread_shift_interval(fp_int, &la, &ua, &lb, &ub, &grid_B);
    ...
    if (iam_B != -1) {
        dread_RHS_factor_twodim(fp_U2, &U2, &grid_B, m_A, &ldu2, &U2_global);
        if (!iam_B) {
            printf("Read U2!\n");
            fflush(stdout);
        }
    ...
    /* ------------------------------------------------------------
       ...
       ------------------------------------------------------------*/
    double* TTcores[3];
    fadi_ttsvd_3d(options, m_A, nnz_A, nzval_A, rowind_A, colptr_A, m_B, nnz_B, nzval_B, rowind_B, colptr_B,
        m_C, nnz_C, nzval_C, rowind_C, colptr_C, &grid_A, &grid_B, &grid_C, U1, ldu1, U2, ldu2, V2, ldv2,
