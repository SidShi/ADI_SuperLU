...
int main(int argc, char *argv[])
{
    ...
    int    iam1, iam2, info, ldu1, ldu2, ldv2, ldt1, ldt2, ldt3;
    char   **cpp, c, *suffix;
    char   postfix[10];
    ...
    if (iam1 != -1) {
        dread_shift_onegrid(fp_shift1, &pp1, &qq1, &ll1, &grid1);
    ...
    if (iam1 != -1) {
        dread_shift_interval(fp_int, &la, &ua, &lb, &ub, &grid1);
    ...
    dcheck_error_TT_2grids(ms, nnzs, nzvals, rowinds, colptrs, &grid1, &grid2, ranks, locals, 3, trueF_global, TTcores, trueX_global, grid_proc);
