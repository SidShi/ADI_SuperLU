...
int main(int argc, char *argv[])
{
    ...
    char   **cpp, c, *suffix;
    char   postfix[10];
    FILE   *fp_A, *fp_B, *fp_C, *fp_D, *fp_E, *fp_shift1, *fp_shift2, *fp_shift3, *fp_shift4, *fp_int;
    FILE   *fp_U1, *fp_U2, *fp_U3, *fp_V1, *fp_V2, *fp_X, *fp_F, *fp_M, *fp_R, *fopen();
    int ii, omp_mpi_level;
    int p; /* The following variables are used for batch solves */
    int_t i, j;
    double *trueX_global, *U1_global, *U2_global, *U3_global, *V1_global, *V2_global, *trueF_global;
    double tol = 1.0/(1e16);
    float result_min[2];
    result_min[0]=1e10;
    result_min[1]=1e10;
	...
	if ( **cpp == '-' ) {
        ...
        strcpy(name_E, *cpp);
        strcat(name_E, buf_E);
        strcat(name_E, postfix);
        if (!(fp_E = fopen (name_E, "r")))
        {
            ABORT ("File for E does not exist");
        }
        ...
        char buf_shift4[3];
        buf_shift4[0] = 'T'; buf_shift4[1] = 'a'; buf_shift4[2] = '\0';
        char name_shift4[100];
        strcpy(name_shift4, *cpp);
        strcat(name_shift4, buf_shift4);
        strcat(name_shift4, postfix);
        if (!(fp_shift4 = fopen (name_shift4, "r")))
        {
            ABORT ("File for fourth sets of shifts does not exist");
        }

        char buf_int[2];
        buf_int[0] = 'J'; buf_int[1] = '\0';
        char name_I[100];
        strcpy(name_I, *cpp);
    ...
    m_A = ms[0]; m_B = ms[1]; m_C = ms[2]; m_D = ms[3]; m_E = ms[4];
    ms_rev[0] = ms[4]; ms_rev[1] = ms[3]; ms_rev[2] = ms[2]; ms_rev[3] = ms[1]; ms_rev[4] = ms[0];
    nrhss_rev[0] = nrhss[3]; nrhss_rev[1] = nrhss[2]; nrhss_rev[2] = nrhss[1]; nrhss_rev[3] = nrhss[0];
    if (iam2 != -1) {
        // ms[0] = ms_rev[0]; ms[1] = ms_rev[1]; ms[2] = ms_rev[2]; ms[3] = ms_rev[3];
        nrhss[0] = nrhss_rev[0]; nrhss[1] = nrhss_rev[1]; nrhss[2] = nrhss_rev[2]; nrhss[3] = nrhss_rev[3];
    }
    ...
    if (iam2 != -1) {
        dread_shift_onegrid(fp_shift4, &(pps[0]), &(qqs[0]), &(lls[0]), &grid2);
        if (!iam2) {
            printf("Read shifts for reversed fourth equation!\n");
            fflush(stdout);
        }
    }

    if ((iam1 != -1) || (iam2 != -1)) {
        dread_shift_twogrids(fp_shift3, &(pps[deal-1]), &(qqs[deal-1]), &(lls[deal-1]), &grid1, &grid2, 0, 1, grid_proc);
        ...
        dread_shift_multi_interval_2grids(fp_int, dim, las_all, uas_all, lbs_all, ubs_all, &grid1, &grid2, grid_proc);

        if (!iam1) {
            las[0] = las_all[0]; uas[0] = uas_all[0]; lbs[0] = lbs_all[0]; ubs[0] = ubs_all[0];
            las[1] = las_all[1]; uas[1] = uas_all[1]; lbs[1] = lbs_all[1]; ubs[1] = ubs_all[1];
        }
        if (!iam2) {
            las[0] = las_all[2]; uas[0] = uas_all[2]; lbs[0] = lbs_all[2]; ubs[0] = ubs_all[2];
        }
    ...
    dread_matrix(fp_D, suffix, &grid2, &mm, &rr, &(nnzs[1]), &(nzvals[1]), &(rowinds[1]), &(colptrs[1]));
    if (!iam2) {
        printf("Read D!\n");
        fflush(stdout);
    }
    if (iam2 != -1) {
        nnzs_alt[3] = nnzs[1];
        dallocateA_dist(m_D, nnzs[1], &(nzvals_alt[3]), &(rowinds_alt[3]), &(colptrs_alt[3]));
        for (i = 0; i < m_D; ++i) {
            for (j = colptrs[1][i]; j < colptrs[1][i+1]; ++j) {
                nzvals_alt[3][j] = nzvals[1][j];
                rowinds_alt[3][j] = rowinds[1][j];
            }
    ...
    if (iam2 != -1) {
        nnzs_alt[4] = nnzs[0];
        dallocateA_dist(m_E, nnzs[0], &(nzvals_alt[4]), &(rowinds_alt[4]), &(colptrs_alt[4]));
        for (i = 0; i < m_E; ++i) {
            for (j = colptrs[0][i]; j < colptrs[0][i+1]; ++j) {
                nzvals_alt[4][j] = nzvals[0][j];
                rowinds_alt[4][j] = rowinds[0][j];
            }
    ...
    /* ------------------------------------------------------------
       ...
       ------------------------------------------------------------*/
    fadi_ttsvd_2way(options, dim, ms, nnzs, nzvals, rowinds, colptrs, &grid1, &grid2, Us, Vs, locals, nrhss, pps, qqs, lls, tol,
