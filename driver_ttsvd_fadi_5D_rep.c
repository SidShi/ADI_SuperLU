...
int main(int argc, char *argv[])
{
    ...
    FILE   *fp_U1, *fp_U2, *fp_U3, *fp_U4, *fp_V1, *fp_V2, *fp_V3, *fp_V4, *fp_X, *fp_F, *fp_M, *fp_R, *fopen();
    int ii, omp_mpi_level;
    int p; /* The following variables are used for batch solves */
    int_t i, j;
    double *pps[dim-1], *qqs[dim-1];
    double *TTcores[dim];
    double *trueX_global, *U1_global, *U2_global, *U3_global, *U4_global, *V1_global, *V2_global, *V3_global, *V4_global, *trueF_global;
	...
	if ( **cpp == '-' ) {
        ...
        char buf_shift4[3];
        buf_shift4[0] = 'S'; buf_shift4[1] = 'd'; buf_shift4[2] = '\0';
        ...
        char buf_U4[3];
        buf_U4[0] = 'U'; buf_U4[1] = 'd'; buf_U4[2] = '\0';
        char name_U4[100];
        strcpy(name_U4, *cpp);
        strcat(name_U4, buf_U4);
        strcat(name_U4, postfix);
        if (!(fp_U4 = fopen (name_U4, "r")))
        {
            ABORT ("File for RHS first factor of fourth unfolding does not exist");
        }
        ...
        char buf_V4[3];
        buf_V4[0] = 'V'; buf_V4[1] = 'd'; buf_V4[2] = '\0';
        char name_V4[100];
        strcpy(name_V4, *cpp);
        strcat(name_V4, buf_V4);
        strcat(name_V4, postfix);
        if (!(fp_V4 = fopen (name_V4, "r")))
        {
            ABORT ("File for RHS second factor of fourth unfolding does not exist");
        }
    ...
    if ((iam1 != -1) || (iam2 != -1)) {
        ...
        dread_shift_twogrids(fp_shift4, &(pps[3]), &(qqs[3]), &(lls[3]), &grid1, &grid2, 0, 1, grid_proc);
        ...
        if (!iam1) {
            printf("Read shifts for all four equations!\n");
            fflush(stdout);
        }
    ...
    if (iam1 != -1) {
        ...
        dread_RHS_factor_multidim(fp_U4, &(Us[3]), &grid1, ms, 3, &(locals1[3]), &U4_global);

        if (!iam1) {
            printf("Read U1, U2, U3, and U4!\n");
            fflush(stdout);
        }
    }
    ...
    if (iam2 != -1) {
        ...
        dread_RHS_factor_multidim(fp_V4, &(Vs[3]), &grid2, ms_rev, 3, &(locals2[3]), &V4_global);

        if (!iam2) {
            printf("Read V1, V2, V3, and V4!\n");
            fflush(stdout);
        }
    }
    ...
    dread_matrix_twogrids(fp_D, suffix, &grid1, &grid2, &mm, &rr, &(nnzs[3]), &(nzvals[3]), &(rowinds[3]), &(colptrs[3]));
    if (!iam1) {
        printf("Read B, C, and D!\n");
        fflush(stdout);
    }
    if ((iam1 != -1) || (iam2 != -1)) {
    ...
    dread_matrix(fp_E, suffix, &grid2, &mm, &rr, &(nnzs[4]), &(nzvals[4]), &(rowinds[4]), &(colptrs[4]));
    if (!iam2) {
        printf("Read E!\n");
        fflush(stdout);
    }
    if (iam2 != -1) {
        dallocateA_dist(m_E, nnzs[4], &(nzvals_alt[4]), &(rowinds_alt[4]), &(colptrs_alt[4]));
        for (i = 0; i < m_E; ++i) {
            for (j = colptrs[4][i]; j < colptrs[4][i+1]; ++j) {
                nzvals_alt[4][j] = nzvals[4][j];
                rowinds_alt[4][j] = rowinds[4][j];
            }
            colptrs_alt[4][i] = colptrs[4][i];
        }
