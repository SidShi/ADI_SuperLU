...
int main(int argc, char *argv[])
{
    ...
    FILE   *fp_U1, *fp_U2, *fp_V1, *fp_V2, *fp_X, *fp_F, *fp_M, *fp_R, *fopen();
    int ii, omp_mpi_level;
    int p; /* The following variables are used for batch solves */
    int_t i, j;
    double *pps[deal], *qqs[deal];
    double *TTcores[dim];
    double *trueX_global, *U1_global, *U2_global, *V1_global, *V2_global, *trueF_global;
    double *Us[deal], *Vs[deal];
    double tol = 1.0/(1e16);
    double las_all[2*(deal-1)], uas_all[2*(deal-1)], lbs_all[2*(deal-1)], ubs_all[2*(deal-1)];
    ...
    if (iam2 != -1) {
        dread_shift_onegrid(fp_shift3, &(pps[0]), &(qqs[0]), &(lls[0]), &grid2);
        if (!iam2) {
            printf("Read shifts for reversed third equation!\n");
            fflush(stdout);
        }
    }
    ...
    if ((iam1 != -1) || (iam2 != -1)) {
        ...
        if (!iam2) {
            las[0] = las_all[1]; uas[0] = uas_all[1]; lbs[0] = lbs_all[1]; ubs[0] = ubs_all[1];
        }
    ...
    dread_matrix(fp_C, suffix, &grid2, &mm, &rr, &(nnzs[1]), &(nzvals[1]), &(rowinds[1]), &(colptrs[1]));
    if (!iam2) {
        printf("Read C!\n");
        fflush(stdout);
    }
    if (iam2 != -1) {
        nnzs_alt[2] = nnzs[1];
        dallocateA_dist(m_C, nnzs[1], &(nzvals_alt[2]), &(rowinds_alt[2]), &(colptrs_alt[2]));
        for (i = 0; i < m_C; ++i) {
            for (j = colptrs[1][i]; j < colptrs[1][i+1]; ++j) {
                nzvals_alt[2][j] = nzvals[1][j];
                rowinds_alt[2][j] = rowinds[1][j];
            }
            colptrs_alt[2][i] = colptrs[1][i];
        }
        colptrs_alt[2][m_C] = colptrs[1][m_C];
    }

    dread_matrix(fp_D, suffix, &grid2, &mm, &rr, &(nnzs[0]), &(nzvals[0]), &(rowinds[0]), &(colptrs[0]));
    if (!iam2) {
        printf("Read D!\n");
        fflush(stdout);
    }
    if (iam2 != -1) {
        nnzs_alt[3] = nnzs[0];
        dallocateA_dist(m_D, nnzs[0], &(nzvals_alt[3]), &(rowinds_alt[3]), &(colptrs_alt[3]));
        for (i = 0; i < m_D; ++i) {
            for (j = colptrs[0][i]; j < colptrs[0][i+1]; ++j) {
                nzvals_alt[3][j] = nzvals[0][j];
                rowinds_alt[3][j] = rowinds[0][j];
            }
            colptrs_alt[3][i] = colptrs[0][i];
        }
        colptrs_alt[3][m_D] = colptrs[0][m_D];
    }
